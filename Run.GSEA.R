...
GSEA.program.location <- "/work/software/gsea/GSEA.1.0.R"   #  R source program (change pathname to the rigth location in local machine)
source(GSEA.program.location, verbose=T, max.deparse.length=9999)

GSEA(                                                                    # Input/Output Files :-------------------------------------------
 input.ds = args[1],           # Input gene expression Affy dataset file in RES or GCT format
 input.cls = args[2],           # Input class vector (phenotype) file in CLS format
 gs.db =     args[3],         # Gene set database in GMT format
 output.directory      = args[4],   # Directory where to store output and results (default: "")
#  Program parameters :-------------------------------------------------------------------------------------------------------------------------
 doc.string            = args[5],   # Documentation string used as a prefix to name result files (default: "GSEA.analysis")
...
GSEA.Analyze.Sets(
   directory = args[4],        # Directory where to store output and results (default: "")
